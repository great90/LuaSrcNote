...
static TValue *index2adr (lua_State *L, int idx) {
  ...
  else if (idx > LUA_REGISTRYINDEX) { // 索引为负，从函数栈顶为起始位置向下查找
...
void luaA_pushobject (lua_State *L, const TValue *o) {
  setobj2s(L, L->top, o); // 将o复制到栈顶
  api_incr_top(L);		  // 栈顶上移一个单位
}
...
LUA_API int lua_checkstack (lua_State *L, int size) {
  int res = 1;
  lua_lock(L);
  if (size > LUAI_MAXCSTACK || (L->top - L->base + size) > LUAI_MAXCSTACK)	// 8000
